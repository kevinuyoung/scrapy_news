...
const dataFromSegmentfault = () => {
  ...
  superAgent
    ...
    .end((err, documents) => {
      ...
      console.log(data);
      fs.writeFile('segmentfault.json', JSON.stringify(data), (err) => {
        if (err) {
          throw new Error('Warning: write segmentfault.json error');
        }
        console.log('write segmentfault.json success');
      })
    })
};
